    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-5 text-slate-800 dark:text-slate-200 text-xs sm:text-sm">
          ...
          {activeStep === 4 && (
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-2">
                <input
                  type="text"
                  placeholder="Buscar bombero por nombre, cargo o registro..."
                  value={volunteerSearch}
                  onChange={(e) => setVolunteerSearch(e.target.value)}
                  className="flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-red-600 focus:outline-none"
                />
          ...
          {/* Form Actions Footer */}
          <div className="bg-slate-50 dark:bg-slate-800/80 -mx-5 -mb-5 px-5 py-3 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
            <div>
              {activeStep > 1 && (
                <button
                  type="button"
                  onClick={() => setActiveStep(activeStep - 1)}
                  className="text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-slate-900 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"
                >
                  Anterior
                </button>
              )}
            ...
            <div className="flex items-center space-x-2">
              <button
                type="button"
                onClick={onClose}
                className="text-xs font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-white px-3 py-2 rounded-lg"
              >
                Cancelar
              </button>
              ...
              {activeStep < 5 ? (
                <button
                  type="button"
                  onClick={() => setActiveStep(activeStep + 1)}
                  className="bg-slate-900 dark:bg-red-700 hover:bg-slate-800 dark:hover:bg-red-800 text-white text-xs font-bold px-4 py-2 rounded-lg shadow-sm"
                >
                  Siguiente paso
                </button>
              ) : (
                <button
                  type="submit"
                  className="flex items-center space-x-1.5 bg-red-700 hover:bg-red-800 text-white text-xs font-bold px-5 py-2 rounded-lg shadow-md transition active:scale-95"
                >
